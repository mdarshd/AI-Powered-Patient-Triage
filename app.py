...
--------------------------------------------------
A real-time Streamlit application that sends patient-submitted symptoms
or lab metrics to an AWS Lambda endpoint for AI-based triage
classification (HIGH / MEDIUM / LOW), then displays a plain-English
explanation and recommended next steps.

The app is fully real-time: no local dataset is used. All data is
submitted to, and retrieved from, the backend on demand.

Backend endpoint is read from the LAMBDA_URL environment variable
(see .env.example) so no credentials or URLs are hardcoded in source.
"""
